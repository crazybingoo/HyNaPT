function hyperEdges = gain_hyperEdges(datanew)
    ...
    if isempty(nonZero_sorted_plv)
        hyperEdges = {};
        return;
    end
    thresholdIndex = max(1, min(numel(nonZero_sorted_plv), ...
        round(numel(nonZero_sorted_plv) * 0.55)));
    threshold = nonZero_sorted_plv(thresholdIndex);
